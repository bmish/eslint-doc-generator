...
| `--config-emoji` | Custom emoji to use for a config. Format is `config-name,emoji`. Option can be repeated. | Default emojis are provided for [common configs](./lib/emojis.ts). Configs without emojis will be omitted from documentation. See [Badges](#badges) for using custom badges as emojis. |
